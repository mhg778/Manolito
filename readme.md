...
+ [!] Módulo OfflineOS: Aísla la identidad en la nube (AAD, MSA,
  Scoobe) incluso si omites la activación por licencia.
+ [!] Gaming Mode (-GamingMode): Consérvate Xbox, pero inyecta 
  HAGS en la GPU y elimina la latencia (aceleración) del ratón.
+ [!] Secure DNS (-SetSecureDNS): Fuerza Cloudflare (1.1.1.1) y 
  Quad9 a nivel de SO en adaptadores físicos.
+ [!] Sysadmin Bootstrapper: Descarga silenciosa vía Winget de 
  7-Zip, PuTTY, Notepad++, Sysinternals y TotalCmd.
+ [!] Win32 Long Paths nativos para evitar crashes en NodeJS/Git.

---[ INSTRUCCIONES DE USO (LEE QUE NO MUERDE) ]-------------------

1. Abre el script manolito.ps1 con tu editor favorito.
2. (Opcional) Busca la sección de CONFIGURACION DE USUARIO y mete 
   tu serial de Windows 11. Si no lo pones, el script te lo pedirá 
...
la sincronización del Candy Crush. Si no sabes lo que hace
